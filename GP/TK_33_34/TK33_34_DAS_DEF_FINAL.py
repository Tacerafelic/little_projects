...
class ImageApp(ctk.CTk):
    ...
    def load_image(
        self, path
    ):  # Hier wird es interessant... Das asugewählte bild wird geöffnet
        self.image = cv.imread(path)
        self.original = self.image.copy()
        self.image = cv.cvtColor(
            self.image, cv.COLOR_BGR2RGB
        )  # weil cv das Bild immer invertiert öffnen möchte. Wenn besser idee bitte her, aber Achtung. in def apply_resize ist der selbe trick nochmal >:3
        ...
        )  # Das Bild wird auf einem Canvas geladen
        self.canvas.grid(row=0, column=1, sticky="nsew")
        self.canvas.create_image(0, 0, anchor="nw", image=self.image_tk)

        self.canvas_fin = Canvas(
            self,
            width=self.image.shape[1],
            height=self.image.shape[0],
            bg="grey",
        ...
        self.canvas_fin.grid(row=1, column=1, sticky="nsew")
        self.canvas_fin.create_image(0, 0, anchor="nw")
