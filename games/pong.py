...
def draw_objects():
    pygame.draw.rect(
        screen, PINK, (int(p1_x_pos), int(p1_y_pos), paddle_width, paddle_height)
    )
    pygame.draw.rect(
        screen, BLUE, (int(p2_x_pos), int(p2_y_pos), paddle_width, paddle_height)
    )
...
def apply_ball_movement():
    ...
    if (ball_x_pos + ball_x_vel > p2_x_pos - paddle_width) and (
        p2_y_pos < ball_y_pos + ball_y_vel < p2_y_pos + paddle_height
    ):
        ball_x_vel = -ball_x_vel
